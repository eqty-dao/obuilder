...
# Runtime Stage (Node version should be the same as build stage)
FROM node:20 AS runtime
RUN apt-get update && apt-get install -y clang zip \
    && apt-get clean \
    && rm -rf /var/lib/apt/lists/*
...
# Copy installed Rust tools and environment setup from build stage
COPY --from=build /root/.cargo /root/.cargo
ENV PATH="/root/.cargo/bin:${PATH}"

# Set the default Rust toolchain in the runtime stage
RUN /root/.cargo/bin/rustup default stable
