...
export class RelayService {
	...
	public async sendFile(content: Uint8Array, sender: Account, recipient: string, requestId: string): Promise<string> {
		try {
			...
			this.loggingService.log(requestId, `Sending File with sender:${sender.address} and recipient:${recipient}`);

			if (!sender || !recipient) {
				this.loggingService.logError(requestId, `Provide the signer and recipient. signer: ${sender?.address}  recipient:${recipient}`);
			...
			this.loggingService.log(requestId, `Message hash: ${message.hash.base58}`);
			this.loggingService.log(requestId, `Message: type${message.type} sender:${JSON.stringify(message.sender)} recipient:${message.recipient} timestamp:${message.timestamp} mediaType:${message.mediaType}`);
			try {
				...
				this.loggingService.log(requestId, `Ownable successfully sent to Relay. Setting Queue status to sent.`);

				if (ltoNetwork === 'L') {
					await this.queueService.setQueueEntryStatus('L', requestId, OwnableStatus.Sent, hashBase58);
				} else {
					await this.queueService.setQueueEntryStatus('T', requestId, OwnableStatus.Sent, hashBase58);
				}
			...
			this.loggingService.logError(requestId, `Send file error: ${error.message}`);
			throw error;
		}
	}
	...
	public async sendOwnable(ltoNetworkId: 'L' | 'T', requestId: string, recipient: string, content?: Uint8Array): Promise<string> {
		...
		this.loggingService.log(requestId, `PRE-SEND: Checking queue entry existence`);
		// console.log("5");
		// this.queueService.showQueue();
		const [entry, index] = this.queueService.getQueueEntryByRequestId(ltoNetworkId, requestId);
		// console.log("6");
		// this.queueService.showQueue();
		this.loggingService.log(requestId, `PRE-SEND: Entry exists: ${!!entry}, index: ${index}`);

		const ltoNetworkIdRecipient = this.getLtoNetwork(recipient);
		this.loggingService.log(requestId, `Sending Ownablefile... RELAY:${relayURL} RECIPIENT:${recipient} RID:${requestId} NETWORKID: ${ltoNetworkId}.`);


		if (ltoNetworkId !== ltoNetworkIdRecipient) {
			this.loggingService.logError(requestId, `Lto NetworkIds of currently produced Ownable ${ltoNetworkId} and recipient ${ltoNetworkIdRecipient} do not match`);
		...
		if (ltoNetworkId == 'L') {
			...
			this.loggingService.logError(requestId, `Unknown ltoNetworkID ${ltoNetworkId}`);
		...
		try {
			...
			if (recipient) {
				...
				const [postEntry, postIndex] = this.queueService.getQueueEntryByRequestId(ltoNetworkId, requestId);
				this.loggingService.log(requestId, `POST-SEND: Entry exists: ${!!postEntry}, index: ${postIndex}`);

				// If entry is missing but file was sent, create a new one with Sent status
				if (!postEntry || postIndex === -1) {
					this.loggingService.logError(requestId, `Queue entry missing after successful send. Creating new entry with Sent status.`);

					// We could recreate an entry here or let the caller handle this
					const queue = ltoNetworkId === 'L' ? this.queueService.queueMainnet : this.queueService.queueTestnet;
					...
					if (!entry) {
						...
						this.loggingService.log(requestId, `Created new entry with Sent status`);
					}
				}

				return hashBase58;
			} else {
				this.loggingService.logError(requestId, `Failed to send Ownable RELAY:${relayURL} SENDER:${sender.address} RECIPIENT:${recipient} RID:${requestId}.`);
				throw new Error("No recipient provided");
			}
		} catch (error) {
			this.loggingService.logError(requestId, `Error sending message: ${error}`);
