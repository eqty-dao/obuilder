...
export class LtoService implements OnModuleInit {
   ...
   */
	public async checkReuseOfTxId(ltoTransactionId: string, requestId: string): Promise<void> {
 ...
 */
	public async checkLtoTransactionId(
		...
		for (let attempt = 0; attempt < maxRetries; attempt++) {
			try {
			  this.loggingService.log(requestId, `HTTP Request sent to: ${url} on LTO network ${ltoNetworkId} (attempt ${attempt + 1}/${maxRetries})`);
			  ...
			  if (data.status === 'error') {
				...
				this.loggingService.log(requestId, `Waiting ${retryDelay/1000} seconds to retry...`);
		...
		if (attempt === maxRetries - 1 || !err.message?.includes('not in blockchain')) {
		  this.loggingService.logError(requestId, `Request Failed with error: ${err}`);
		  throw new Error(err);
		}
		
		// Log the error but continue with retry
		this.loggingService.log(requestId, `Transaction not found yet, retrying in ${retryDelay/1000} seconds...`);
		await new Promise(resolve => setTimeout(resolve, retryDelay));
	  }
	}
}
