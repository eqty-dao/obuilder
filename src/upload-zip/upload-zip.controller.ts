...
export class UploadZipController {
  ...
  async uploadFile(
    ...
    if (
      ...
      Object.prototype.isPrototypeOf(file) == false
    ) {
    ...
    if (inputUploadFile.templateId !== undefined) {
      ...
      const templateExists =
        await this.uploadZipService.checkTemplateExists(templateId);
      if (!templateExists) {
  ...
  getQueueEntriesByRequestId(
    @Query('requestId') requestId: string,
    @Query('ltoNetworkId') ltoNetworkId: 'L' | 'T',
  ) {
    return this.uploadZipService.getQueueEntriesByRequestId(
      ltoNetworkId,
      requestId,
    );
  }
  ...
  getQueueEntriesByStatus(
    @Query('status') status: OwnableStatus,
    @Query('ltoNetworkId') ltoNetworkId: 'L' | 'T',
  ) {
  ...
  async resendOwnableByRequestId(
    @Query('requestId') requestId: string,
    @Query('ltoNetworkId') ltoNetworkId: 'L' | 'T',
  ) {
    return await this.uploadZipService.resendOwnableByRequestId(
      ltoNetworkId,
      requestId,
    );
  }
  ...
  async getTemplatePreview(@Param('id') id: string, @Res() res: Response) {
    ...
    const templateInfo =
      await this.uploadZipService.getTemplatePreview(templateId);
  ...
  getServerLtoWalletAddresses() {
    try {
      const [serverWalletAddressLTO_L, serverWalletAddressLTO_T] =
        this.uploadZipService.getServerLtoWalletAddresses();
      return {
  ...
  async GetServerInfo() {
    try {
      ...
      const balanceARB_T = await this.uploadZipService.GetServerETHBalance(
        'T',
        'arbitrum',
      );
      const balanceLTO_L =
        await this.uploadZipService.getLTOAccountBalance('L');
      const balanceLTO_T =
        await this.uploadZipService.getLTOAccountBalance('T');
      ...
      const [serverWalletAddressEVM_L, serverWalletAddressEVM_T] =
        this.uploadZipService.getServerEVMwalletAddresses('arbitrum');
