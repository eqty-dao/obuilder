...
export class EthereumService {
  ...
  public getEvmWalletAddresses(): [string, string] {   
    return this.ethers.getEvmWalletAddresses();
  }
  ...
  public async getOwnerOfNFT(ltoNetworkId: 'L' | 'T', nft: NftInfo): Promise<string> {
    return await this.ethers.getOwnerOfNFT(ltoNetworkId, nft);
  }  
  ...
  public async mintNFT(ltoNetworkId: 'L' | 'T', nftReceiverAddress: string, nftTokenURI: string, nftInfo: NftInfo): Promise<number> {
    return this.ethers.mintNFT(ltoNetworkId, nftReceiverAddress, nftTokenURI, nftInfo);    
  }
