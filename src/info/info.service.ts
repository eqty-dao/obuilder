...
export class InfoService implements OnModuleInit {
  ...
  async getQueueRequestIDs(): Promise<string[]> {
    const requestKeys = await this.redisService.getKeys('request:*');
    return requestKeys.map((key) => key.split(':')[1]);
  }

  public async GetServerETHBalance(): Promise<[string, string]> {
    return await this.nftService.getServerETHBalance();
  }
  ...
  getRelayUrl(): string {
    return (
      this.configService.get('lto.relay') ||
      this.configService.get('lto.local_relay')
    );
  }

  async isRelayServerUp(): Promise<string> {
    const relayURL = this.getRelayUrl();
    try {
      const response = await this.httpService.axiosRef.head(relayURL);
      return response.status === 200
        ? `SUCCESS: oRelay Server ${relayURL} is up and running!`
        : `FAILURE: oRelay Server ${relayURL} is down.`;
    } catch {
      throw new Error(`Relay Server ${relayURL} is down.`);
    }
  }

  isEVMAddress(address: string): boolean {
    return this.nftService.isEVMAddress(address);
  }

  isValidLtoAddress(address: string): boolean {
    return this.lto.isValidAddress(address);
  }

  async getAvailableNftChains(): Promise<Record<string, any>> {
    const nftInfoETH: NftInfo = {
      network: 'ethereum',
      id: 0,
      address: this.configService.get('eth.contracts.ethereum'),
    };
    const nftInfoARB: NftInfo = {
      network: 'arbitrum',
      id: 0,
      address: this.configService.get('eth.contracts.arbitrum'),
    };

    const nftCountETH = await this.nftService.getNFTcount(nftInfoETH);
    const nftCountARB = await this.nftService.getNFTcount(nftInfoARB);

    return {
      ethereum: {
        name: 'ethereum',
        logo: 'https://obuilderassets.s3.eu-west-1.amazonaws.com/ethereum-eth-logo.png',
        smartContractAddress: nftInfoETH.address,
        totalAmountNFTs: nftCountETH.toString(),
        templateCost: this.packageInfo.templateCost.ethereum[1],
      },
      arbitrum: {
        name: 'arbitrum',
        logo: 'https://obuilderassets.s3.eu-west-1.amazonaws.com/arbitrum-arb-logo.png',
        smartContractAddress: nftInfoARB.address,
        totalAmountNFTs: nftCountARB.toString(),
        templateCost: this.packageInfo.templateCost.arbitrum[1],
      },
    };
  }

  templateCost(templateId: number) {
    if (templateId != 1) {
      throw new Error(`Currently only Template ID 1 is supported`);
    }
    return {
      ethereum: this.packageInfo.templateCost.ethereum[templateId],
      arbitrum: this.packageInfo.templateCost.arbitrum[templateId],
    };
  }

  getLTOAccountAddress(): string {
    return this._ltoAccount ? this._ltoAccount.address : '';
  }

  async getLTOAccountBalance(address?: string) {
    const url = `${this.configService.get('lto.node')}/addresses/balance/${
      address || this.getLTOAccountAddress()
    }`;
